(function () {
  ...
  var config = {
      ...
      cookie_exp: 182 * 24,
      if_url: "https://app-automation-staging.51b.dev/cm.html?key=" + ck,
      is_secure: "false",
  ...
  function br(d) {
    ...
    initOptions.email_id && (td.email_id = initOptions.email_id);
    return mo(td, d);
  }
  function generateReqBody(event_type) {
    ...
    if (initOptions.email_id) b.email_id = initOptions.email_id;
    return b;
  }
  ...
  function load_cm(c) {
    ...
    if (
      ...
      const session_url = config.base_url + "/visitor/" + ck,
        x = new XMLHttpRequest();
  ...
  function wonderPushTracker() {
    ...
    let wpConfig = { webKey: config.wonderpush_webkey };
    if (initOptions.service_worker_url) {
      wpConfig.serviceWorkerUrl = initOptions.service_worker_url;
    }
    window.WonderPush = window.WonderPush || [];
